...
if ($_COOKIE["session"]) {
    ...
    if ($permissions["editRole"]) {
        ...
        if ($_POST["type"] === "create") {
            ...
            $sql = "INSERT INTO role (" . rtrim($columns, ", ") . ") VALUES (" . rtrim($values, ", ") . ");";

            if ($conn->query($sql)) {
                echo "$('#createBtn').attr('class', 'btn btn-success').html('Role Created')";
            }
            else {
            ...
            $sql = "UPDATE role SET" . rtrim($set, ", ") . " WHERE id = " . $_POST["id"] . ";";

            if ($conn->query($sql)) {
                echo "$('#editBtn').attr('class', 'btn btn-success').html('Role Edited').removeAttr('disabled');";
            }
            else {
